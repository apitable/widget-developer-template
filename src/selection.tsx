  ...
  return (
    <div>
      ...
      <p>
        Active cell:{" "}
        {(activeCell?.fieldId &&
          activeRecord?.getCellValueString(activeCell?.fieldId)) ||
          "_"}
      </p>
      <p>选区内容</p>
      <table>
        <thead>
          <tr>
            {fields.map((field) => (
              <th key={field.id} style={{ border: "solid 1px gray" }}>
                {field.name || "_"}
              </th>
            ))}
          </tr>
        ...
        <tbody>
          {records.map((record) => (
            <tr key={record.id}>
              {fields.map((field) => (
                <td key={field.id} style={{ border: "solid 1px gray" }}>
                  {record.getCellValueString(field.id)?.slice(0, 8) || "_"}
                </td>
              ))}
            </tr>
          ))}
