  ...
  return (
    <div>
      ...
      <p>
        First record:{" "}
        {allRecords.length &&
          allRecords[0].getCellValueString(primaryField!.id)}{" "}
        <Button
          size="small"
          onClick={() =>
            expandRecord({
              viewId,
              recordIds: [allRecords.length && allRecords[0].id],
            })
          }
        >
          Expand
      ...
      <p>
        WidgetFullscreen: {isFullscreen + ""}{" "}
        <Button size="small" onClick={() => toggleFullscreen()}>
          Toggle
        </Button>
      </p>
      <p>
        WidgetSettingOpened（Only allow opening in expanded mode）:{" "}
        {isShowingSettings + ""}
        <Button size="small" onClick={() => toggleSettings()}>
          Toggle
        </Button>
      </p>
    </div>
